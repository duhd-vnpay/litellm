# VNPAY LLM Gateway Ops

VNPay-specific deployment, routing hook, và operational config cho LiteLLM Proxy. Folder này nằm trong repo nội bộ [git.vnpay.vn/duhd/litellm](https://git.vnpay.vn/duhd/litellm) (mirror upstream BerriAI/litellm).
...
| 2026-04-23 | UI zombie state: hiển thị `0 results` trên mọi tab (Virtual Keys, Teams, Policies...) sau 8h không dùng | `response.set_cookie("token", ...)` trong `/vnpay-sso` + `/teleport-sso` không set `max_age` → session cookie (browser giữ vĩnh viễn). Backend token expire sau `LITELLM_UI_SESSION_DURATION=8h` → mọi XHR 401 nhưng cookie vẫn còn → nginx handoff rule (`$cookie_token=""`) không trigger redirect SSO | Set `max_age=LITELLM_UI_SESSION_MAX_AGE` (8h, khớp TTL token) + `secure=True` + `samesite="lax"` cho cả 2 handler. `httponly=False` giữ nguyên vì UI đọc cookie qua JS để inject Bearer header. Browser auto-drop cookie đúng chu kỳ → nginx redirect về SSO tạo session mới |
| 2026-04-22 | Moonshot Kimi K2.5 100% fallback → MiniMax từ 13:00 VN (peak hour) | Project `proj-c81621352fd24dde8974050ecfa1a9a1` consumption budget = $30/daily (default Moonshot dashboard). 3 API keys cùng 1 project → share budget → vượt $30 lúc 06:00 UTC → Moonshot trả 429 "exceeded consumption budget" cho cả 3 keys. LiteLLM coi là RateLimit, cooldown 60s — vô nghĩa vì budget reset theo ngày, không theo phút | Tăng budget $30 → $300/daily. Tạo 3 project Moonshot độc lập, mỗi project 1 key → break budget silo. Rotate 2/3 API keys trong DB (8c0cc6b5 giữ, f946d522 + 453ea5a2 key mới). Total capacity 3 × $300 = $900/day. Account balance (> $1000) không liên quan — project budget là cap per-period riêng biệt |
...
- **2026-04-23**: Migrate repo `github.com/duhd-vnpay/litellm` → `git.vnpay.vn/duhd/litellm` (internal GitLab). Lý do: IP compliance (code chứa Teleport SSO, Splunk audit, premium unlock patches, routing nội bộ), supply-chain giảm phụ thuộc GitHub OAuth/CI, ownership từ personal fork sang group VNPay. Upstream BerriAI/litellm sync qua GitLab pull mirror. GitHub fork archive read-only 30 ngày rollback window.
